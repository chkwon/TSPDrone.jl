...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.init_embed = nn.Linear(node_dim, embedding_dim).to(self.device)
...
class Attention(nn.Module):
    ...
    def __init__(self, hidden_size, mode, dev, use_tahn=False, C = 10):
...
class Decoder(nn.Module):
    ...
    def __init__(self, hidden_size, mode, dev, num_layers=1, dropout=0.1, n_glim=0):
        ...
        self.encoder_attn = Attention(hidden_size, mode, dev)
...
class Actor(nn.Module):
    ...
    def __init__(self, hidden_size, mode, dev,
...
class AttentionCritic(nn.Module):
    ...
    def __init__(self, hidden_size, dev, use_tahn=False, C = 10):
...
class Critic(nn.Module):
    ...
    def __init__(self, hidden_size, dev, n_processes=3, num_layers=1):
   ...
   #     self.static_w = Encoder(1, hidden_size)
        ...
        self.attention1 = AttentionCritic(hidden_size, dev)
        self.attention2 = AttentionCritic(hidden_size, dev)
        self.attention3 = AttentionCritic(hidden_size, dev)
        if torch.cuda.is_available():
        ...
        self.fc1 = nn.Linear(self.hidden_size, self.hidden_size).to(self.device)
        self.fc2 = nn.Linear(self.hidden_size, 1).to(self.device)
